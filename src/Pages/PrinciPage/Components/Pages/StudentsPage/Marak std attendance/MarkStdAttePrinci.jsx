...
const SAMPLE_ATTENDANCE_RECORDS = {
  // Format: "classId-date": { studentId: status }
  "1-2025-03-15": { 101: "present", 102: "absent", 103: "present" },
  "2-2025-03-15": { 201: "present", 202: "present" },
  "3-2025-03-15": { 301: "absent" },
  "1-2025-03-14": { 101: "present", 102: "present", 103: "absent" },
  "2-2025-03-14": { 201: "absent", 202: "present" },
};
...
const MarkStdAttePrinci = () => {
  ...
  useEffect(() => {
    ...
    const loadAttendance = () => {
      ...
      // Simulate API delay
      setTimeout(() => {
        const key = `${selectedClass.id}-${date}`
        const savedAttendance = SAMPLE_ATTENDANCE_RECORDS[key]
        
        if (savedAttendance) {
          setAttendance(savedAttendance)
        } else {
          // No records found - set all to absent
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      try {
        ...
        setSuccess(`Attendance submitted successfully! Present: ${presentCount}, Absent: ${selectedClass.students.length - presentCount}`)
        setError(null)
  ...
  const getImageUrl = (url) => {
    if (!url) return 'https://ui-avatars.com/api/?name=User&background=2563eb&color=fff&size=96&bold=true'
    return url
  }
