...
function AddFeeTchr() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setTimeout(() => {
      setAcademicYear(SAMPLE_ACADEMIC_YEAR)
  ...
  const confirmSubmitFee = async () => {
    ...
    try {
      ...
      setNote('')
      setPendingFee(null)
      setSelectedStudent(null) // Optionally clear selected student after successful submission
    } catch (err) {
      setError('Error adding fee. Please try again.')
    } finally {
      setLoading(false)
            ...
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3">
              {students.map((student, index) => (
                <div key={student.id} className="flex flex-col">
                  ...
                  {/* Fee Form - directly below the clicked student's card */}
